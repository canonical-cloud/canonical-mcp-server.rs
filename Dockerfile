...
#
# Multi-stage image for canonical-mcp-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t canonical-mcp-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" canonical-mcp-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=canonical-mcp-server-target-${TARGETARCH},sharing=locked \
    cargo build --release --locked --bin canonical-mcp-server \
    && strip "target/release/canonical-mcp-server" \
    && cp "target/release/canonical-mcp-server" "/usr/local/bin/canonical-mcp-server"
...
COPY --from=build "/usr/local/bin/canonical-mcp-server" "/usr/local/bin/canonical-mcp-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/canonical-mcp-server
ENV SOPS_SECRETS_FILE=/app/secrets/app.env \
    OTEL_SERVICE_NAME=canonical-mcp-server \
    OTEL_EXPORTER_OTLP_ENDPOINT=http://dd-otel-collector.observability.svc.cluster.local:4317 \
    RUST_LOG=info
USER 65532:65532
EXPOSE 8080
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/canonical-mcp-server"]
