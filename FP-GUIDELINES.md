...
check was introduced: **36 findings across 14 files
and 3,203 lines**. CI compares against that budget and fails only when a
...
|---|---:|---|---|---|
| `RS001` | 18 | warn | immutable values | mutable local binding (`let mut`) |
| `RS003` | 8 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
...
*typed errors* · 8 occurrences at baseline
