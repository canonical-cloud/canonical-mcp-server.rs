...
check was introduced: **33 findings across 13 files
and 3,043 lines**. CI compares against that budget and fails only when a
...
| `RS003` | 7 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
...
*typed errors* · 7 occurrences at baseline
